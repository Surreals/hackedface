...
A grid of "pixels" is generated based on the data supplied in the screenDimensions and pixelSize variables. For example, the default setting x:80 & y:45 generates an 80 by 45 grid. 

pixelSize generates smaller or larger "pixels", but it's hard to tell because the canvas is stretched to fit the whole screen. Change that in the .css to see the effect.

The locations of these "pixels" are stored in the "pixels" array by the "calcPixelLocations" function. 

Each pixel will have three subpixels: one for the red channel, one for the green channel, and one for the blue channel. We can control the amount of color in each subpixel.

The "calcGradientRGB" function determines RGB values from the initial hex values we supply. It does this for two colors so we can draw a gradient between them. The RGB values for each color will need to be split among the 3 channels of each pixel's subpixels.

The "assignPixelColor" function then cycles through every pixel and assigns colors to each of their 3 subpixels. It only assigns them, it does not draw or render anything yet.

Then the "draw" function fills in every subpixel with the colors we previously assigned. This is where the colors actually are rendered.

You can change channels by clicking or pressing space. 
Channels are: 
...
let screenMessage = "CLICK OR PRESS SPACE"; 
...
function changeChannel() {
  ...
  if (currentChannel === 1) {
    ...
    screenMessage = "01 COMPLIMENTARY COLORS";
    ...
    //triadic colors
    screenMessage = "02 TRIADIC COLORS";
    let randomHue = getRandom(0, 359);
    let randomSaturation = getRandom(85, 100);
    let randomBrightness = getRandom(40, 60);
    color1.hex = hslToHex(randomHue, randomSaturation, randomBrightness);
    ...
    //square colors
    screenMessage = "03 SQUARE COLORS";
    let randomHue = getRandom(0, 359);
    let randomSaturation = getRandom(85, 100);
    let randomBrightness = getRandom(40, 60);
    color1.hex = hslToHex(randomHue, randomSaturation, randomBrightness);
